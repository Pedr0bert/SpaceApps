...
class LocationService {
  ...
  Future<List<Map<String, String>>> getStatesOfCountry(
    ...
    final url = Uri.parse('$_baseUrl/countries/states/q?country=$countryName');
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
        'Falha ao carregar os estados - Status: ${response.statusCode}',
      );
    }
  }
  ...
  Future<Map<String, double>?> getCityCoordinates(String cityName) async {
    ...
    final url = Uri.parse(
      '$baseUrl?name=$cityName&count=1&language=pt&format=json',
    );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Falha ao carregar as coordenadas - Status: ${response.statusCode}',
        );
      }
    } catch (e) {
      rethrow;
